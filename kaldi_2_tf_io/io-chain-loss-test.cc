...
#define TEST_TF
	...
	chain_config.leaky_hmm_coefficient = 0.1;
	chain_config.l2_regularize = 0.00005;
	bool use_xent = false;

	//  read nnetout
	SequentialBaseFloatMatrixReader nnetout_reader(nnet_rxfilename);
	//nnetout_reader.Next();
	const Matrix<BaseFloat> &nnet_output = nnetout_reader.Value();
...
*/		
		int32 out_frames = nnet_output.NumRows();
		CuMatrix<BaseFloat> nnet_output_gpu_tmp(nnet_output.NumRows(),nnet_output.NumCols(), 
				kUndefined, kStrideEqualNumCols);
		nnet_output_gpu_tmp.CopyFromMat(nnet_output);
		...
		Matrix<BaseFloat> tmp_cpu(nnet_output_deriv);
		...
		// batch test
		{
			...
			for (int32 t = 0; t < out_frames; t++) 
			{
				for(int32 i =0 ; i < batch_size; i++)
				{
					...
					row.CopyFromVec(nnet_output.Row(t));
					//nnet_computer.GetOutputForFrame(t, &row);
				}
			}
