...
int main(int argc, char *argv[]) {
  ...
  try {
    const char *usage =
      ...
      "<ali-map-file> <transition-model-in> <feature-rspecifier> "
...
//    trn_opts.Register(&po);
    ...
    std::string ali_map_filename = po.GetArg(1),
        transition_model_filename = po.GetArg(2),
        feature_rspecifier = po.GetArg(3),
        den_lat_rspecifier = po.GetArg(4),
        num_ali_rspecifier = po.GetArg(5);

	int32 transition_id_num =0;
	const int32 *trans = ReadMap(ali_map_filename, &transition_id_num, 3);
	const int32 *pdf_to_phone = MapConvert(trans, transition_id_num, 3);
...
*/
    ...
    Matrix<BaseFloat> feats_transf, nnet_out, nnet_diff;
    ...
    for (; !feature_reader.Done(); feature_reader.Next()) {
      ...
      if (static_cast<MatrixIndexT>(num_ali.size()) != mat.NumRows()) {
	  ...
	  if (!do_smbr)
		  criterion = "mpfe";
	  bool ret = hubo::MPELoss(indexs, pdf_values, lm_ws, am_ws, statesinfo,
			  batch_num_states, max_num_arcs, max_num_states,
			  h_nnet_out_h,
			  nnet_out_h.NumRows(), batch_size, nnet_out_h.NumCols(),
			  batch_pdf_ali,
			  batch_sequence_length,
			  static_cast<const int32 *>(silence_phones.data()),
			  silence_phones.size(),
			  pdf_to_phone,
			  transition_id_num,
			  old_acoustic_scale,
			  acoustic_scale, gradient,
			  batch_loss, 
			  one_silence_class,
			  criterion);
	  ...
	  if(true)
	  {
		  for(int r=0;r<nnet_out_h.NumRows();r++)
		  {
			  for(int c=0;c<nnet_out_h.NumCols();c++)
			  {
				  if(!ApproxEqual(gradient[nnet_out_h.NumCols()*r+c], nnet_diff(r,c), 1e-5))
				  {
					  std::cout <<  "row " << r << " col " << c << " : "
						  << gradient[nnet_out_h.NumCols()*r+c]  << " != "
						  << nnet_diff(r,c) << std::endl;
				  }
				  //std::cout << gradient[nnet_out_h.NumCols()*r+c] << " ";
			  }
			  //std::cout << std::endl;
...
*/
    ...
    KALDI_LOG << "Done " << num_done << " files, "
              << num_no_num_ali << " with no reference alignments, "
