...
class GlobalHeader(object):
    ...
    def read(fd, type='CM', endian='<'):
        min_value = struct.unpack(str(endian + 'f'), fd.read(4))[0]
        range = struct.unpack(str(endian + 'f'), fd.read(4))[0]
    ...
    def write(self, fd, endian=None):
        ...
        fd.write(struct.pack(str(endian + 'f'), self.min_value))
        fd.write(struct.pack(str(endian + 'f'), self.range))
        fd.write(struct.pack(str(endian + 'i'), self.rows))
        fd.write(struct.pack(str(endian + 'i'), self.cols))
        return self.size
